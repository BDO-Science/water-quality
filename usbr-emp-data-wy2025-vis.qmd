...
missing_flag <- 
readRDS(here("data/data_clean/usbr_contdata_wy2024_missingflag.rds"))
...
-   While EC was collected at the 15-minute level, SpC required water temperature, which was collected at the hourly level.
-   At this stage we converted data to NA if \>30% of daily data was missing

\newpage

```{r, tab.cap = "Percent of data removed due to daily missing values"}
vals_removed <- missing_flag %>%
  group_by(Site, Station, Analyte) %>%
  summarize(removed = sum(missing_flag == 1L, na.rm = TRUE),
            values = n(),
            nonNA_vals = sum(!is.na(Value)),
            pct_removed = if_else(nonNA_vals>0, 
                                  removed/nonNA_vals*100,
                                  0))

kableExtra::kable(vals_removed) %>%
  kable_styling(c("striped", "condensed"),
                full_width = F,
                latex_options = "striped")
```
